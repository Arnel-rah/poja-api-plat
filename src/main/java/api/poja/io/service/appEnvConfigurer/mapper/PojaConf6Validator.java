...
public class PojaConf6Validator implements PojaConfValidator<PojaConf6> {
  ...
  private static final Pattern envVarNameRegexPattern = Pattern.compile("^[A-Za-z][A-Za-z0-9_]*$");
  private static final int ENV_VAR_MAX_NAME_LENGTH = 100;
  ...
  static void validateEnvVars(List<EnvVar> envVars, Set<String> exceptionMessages) {
    if (envVars.isEmpty()) return;
    for (int i = 0; i < envVars.size(); i++) {
      validateEnvVar(envVars.get(i), i, exceptionMessages);
    }
    var envVarNames = envVars.stream().map(e -> e.getName().toUpperCase()).toList();
    var uniqueVarNames = envVarNames.stream().collect(Collectors.toUnmodifiableSet());
    if (envVarNames.size() != uniqueVarNames.size()) {
      exceptionMessages.add("general.custom_java_env_vars contains duplicate names.");
    }
  }

  private static void validateEnvVar(EnvVar envVar, int index, Set<String> exceptionMessages) {
    var prefix = "general.custom_java_env_vars[" + index + "]";
    var name = envVar.getName();

    if (name == null || isBlank(name)) {
      exceptionMessages.add(prefix + ".name is mandatory and cannot be null or empty.");
      return;
    }

    if (envVar.getValue() == null) {
      exceptionMessages.add(prefix + ".value is mandatory.");
    }

    if (name.length() > ENV_VAR_MAX_NAME_LENGTH) {
      exceptionMessages.add(
          prefix + ".name cannot exceed " + ENV_VAR_MAX_NAME_LENGTH + " characters.");
      return;
    }

    if (!envVarNameRegexPattern.matcher(name).matches()) {
      exceptionMessages.add(
          prefix
  ...
  public void accept(@Nullable PojaConf5 from, PojaConf6 to, boolean isPremium) {
    var invalidAttributes = invalidAttributes(to, isPremium);
    var illegalTransitions = outputIllegalTransitions(from, to);
    invalidAttributes.addAll(illegalTransitions);
  ...
  public Set<String> outputIllegalTransitions(@Nullable PojaConf5 from, PojaConf6 to) {
    if (from == null) {
      return Set.of();
    }
    Set<String> illegalTransitions = new HashSet<>();
