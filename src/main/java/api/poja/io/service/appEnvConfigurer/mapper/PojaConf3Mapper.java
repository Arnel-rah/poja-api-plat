...
@Service
final class PojaConf3Mapper extends AbstractAppEnvConfigMapper {
  private final PojaConf3Validator validator;

  PojaConf3Mapper(
      @Qualifier("yamlObjectMapper") ObjectMapper yamlObjectMapper,
      NetworkingService networkingService,
      PojaConf3Validator validator) {
  ...
  public File writeToTempFile(PojaConf pojaConf) {
    var domainPojaConf = ((PojaConf3) pojaConf);
    File namedTempFile =
  ...
  public OneOfPojaConf readAsRest(File file) {
    api.poja.io.endpoint.rest.model.PojaConf3 pojaConf;
    try {
      var domain = yamlObjectMapper.readValue(file, PojaConf3.class);
  ...
  public PojaConf readAsDomain(File file) {
    try {
      return yamlObjectMapper.readValue(file, PojaConf3.class);
    } catch (IOException e) {
      throw new ApiException(SERVER_EXCEPTION, e);
    }
  }
  ...
  public File applyMigration(PojaConf from, PojaConf to, boolean isPremium) {
    PojaConf3 pojaConf = ((PojaConf3) to);
    if (from == null) {
      PojaConf3 NULL = null;
  ...
  private api.poja.io.endpoint.rest.model.PojaConf3 toRest(PojaConf3 domain) {
    ...
    return new api.poja.io.endpoint.rest.model.PojaConf3()
