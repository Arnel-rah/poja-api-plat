...
@Service
final class PojaConf5Mapper extends AbstractAppEnvConfigMapper {
  private final PojaConf5Validator validator;

  PojaConf5Mapper(
      @Qualifier("yamlObjectMapper") ObjectMapper yamlObjectMapper,
      NetworkingService networkingService,
      PojaConf5Validator validator) {
  ...
  public File writeToTempFile(PojaConf pojaConf) {
    var domainPojaConf = ((PojaConf5) pojaConf);
    File namedTempFile =
  ...
  public OneOfPojaConf readAsRest(File file) {
    api.poja.io.endpoint.rest.model.PojaConf5 pojaConf;
    try {
      var domain = yamlObjectMapper.readValue(file, PojaConf5.class);
  ...
  public PojaConf readAsDomain(File file) {
    try {
      return yamlObjectMapper.readValue(file, PojaConf5.class);
    } catch (IOException e) {
      throw new ApiException(SERVER_EXCEPTION, e);
    }
  }
  ...
  public File applyMigration(PojaConf from, PojaConf to, boolean isPremium) {
    PojaConf5 pojaConf = ((PojaConf5) to);
    if (from == null) {
      PojaConf5 NULL = null;
  ...
  private api.poja.io.endpoint.rest.model.PojaConf5 toRest(PojaConf5 domain) {
    ...
    return new api.poja.io.endpoint.rest.model.PojaConf5()
