...
@Service
final class PojaConf8Mapper extends AbstractAppEnvConfigMapper {
  private final PojaConf8Validator validator;

  PojaConf8Mapper(
      @Qualifier("yamlObjectMapper") ObjectMapper yamlObjectMapper,
      NetworkingService networkingService,
      PojaConf8Validator validator) {
  ...
  public File writeToTempFile(PojaConf pojaConf) {
    var domainPojaConf = ((PojaConf8) pojaConf);
    File namedTempFile =
  ...
  public OneOfPojaConf readAsRest(File file) {
    api.poja.io.endpoint.rest.model.PojaConf8 pojaConf;
    try {
      var domain = yamlObjectMapper.readValue(file, PojaConf8.class);
  ...
  public PojaConf readAsDomain(File file) {
    try {
      return yamlObjectMapper.readValue(file, PojaConf8.class);
    } catch (IOException e) {
      throw new ApiException(SERVER_EXCEPTION, e);
    }
  }
  ...
  public File applyMigration(PojaConf from, PojaConf to, boolean isPremium) {
    PojaConf8 pojaConf = ((PojaConf8) to);
    if (from == null) {
      PojaConf8 NULL = null;
  ...
  private api.poja.io.endpoint.rest.model.PojaConf8 toRest(PojaConf8 domain) {
    List<PojaConf8.ScheduledTask> scheduledTasks = domain.scheduledTasks();
    ...
    return new api.poja.io.endpoint.rest.model.PojaConf8()
        ...
        .scheduledTasks(
            scheduledTasks == null
                ? null
                : scheduledTasks.stream().map(PojaConf8.ScheduledTask::toRest).toList())
