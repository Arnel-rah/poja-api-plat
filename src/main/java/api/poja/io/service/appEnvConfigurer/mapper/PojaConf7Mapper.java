...
@Service
final class PojaConf7Mapper extends AbstractAppEnvConfigMapper {
  private final PojaConf7Validator validator;

  PojaConf7Mapper(
      @Qualifier("yamlObjectMapper") ObjectMapper yamlObjectMapper,
      NetworkingService networkingService,
      PojaConf7Validator validator) {
  ...
  public File writeToTempFile(PojaConf pojaConf) {
    var domainPojaConf = ((PojaConf7) pojaConf);
    File namedTempFile =
  ...
  public OneOfPojaConf readAsRest(File file) {
    api.poja.io.endpoint.rest.model.PojaConf7 pojaConf;
    try {
      var domain = yamlObjectMapper.readValue(file, PojaConf7.class);
  ...
  public PojaConf readAsDomain(File file) {
    try {
      return yamlObjectMapper.readValue(file, PojaConf7.class);
    } catch (IOException e) {
      throw new ApiException(SERVER_EXCEPTION, e);
    }
  }
  ...
  public File applyMigration(PojaConf from, PojaConf to, boolean isPremium) {
    PojaConf7 pojaConf = ((PojaConf7) to);
    if (from == null) {
      PojaConf7 NULL = null;
  ...
  private api.poja.io.endpoint.rest.model.PojaConf7 toRest(PojaConf7 domain) {
    ...
    return new api.poja.io.endpoint.rest.model.PojaConf7()
